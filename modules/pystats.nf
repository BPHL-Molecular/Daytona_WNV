process pystats {
    ...
    with open("${mypath}"+"/report.txt", 'w') as report:
        header = ['sampleID', 'k_species', 'k_percent', 'reference', 'start', 'end', 'num_raw_reads', 'num_clean_reads', 'num_mapped_reads', 'percent_mapped_clean_reads', 'cov_bases_mapped', 'percent_genome_cov_map', 'mean_depth', 'mean_base_qual', 'mean_map_qual', 'assembly_length', 'numN', 'percent_ref_genome_cov', 'VADR_flag', 'QC_flag']
        report.write('\t'.join(map(str,header)) + '\n')
        results = [items[-1], tax, percent, ref_name, start, end, raw_reads, clean_reads, reads_mapped, percent_map, cov_bases, cov, depth, baseq, mapq, num_bases, ns, pg, vadr_flag, qc_flag]
